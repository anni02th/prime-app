...
router.put("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    // If only updating the primary advisor
      ...
      if (!student.advisors) {
        student.advisors = [req.body.advisor]
      } else if (!student.advisors.includes(req.body.advisor)) {
        student.advisors.push(req.body.advisor)
      }
    }
    ...
    // Populate advisor and advisors information
    await updatedStudent.populate("advisor", "name email avatar")
...
router.patch("/:id/advisors", authenticateToken, isAdminOrAdvisor, async (req, res) => {
  try {
    ...
    if (action === "add") {
      ...
      if (student.advisor && student.advisor.toString() === advisorId) {
        student.advisor = student.advisors.length > 0 ? student.advisors[0] : null
      }
    }
